...
void TouchHMI::draw_result_panel(cv::Mat& canvas, int rpx, int rpy, int rph,
    ...
    if (roi_cal) {
        ...
        const char* edge_names[] = {"TOP", "RIGHT", "BOTTOM", "LEFT"};
        draw_row("EDGE", edge_names[svc_->state.roi_params.selected_edge], Theme::YELLOW);
        for (int i = 0; i < 4; ++i) {
            snprintf(buf, sizeof(buf), "%.0f, %.0f", pts[i].x, pts[i].y);
            draw_row("P" + std::to_string(i + 1), buf, Theme::TXT2);
        }
        draw_row("COORD", "1920 x 1080", Theme::CYAN);
        draw_row("ACTION", "DRAG 4 EDGES", Theme::TXT);
...
bool TouchHMI::handle_roi_pointer(int event, int x, int y) {
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
        ...
        int nearest = -1;
        double nearest_distance = 55.0;
        for (int i = 0; i < 4; ++i) {
            const int next = (i + 1) % 4;
            cv::Point2f a(
                displayed_frame_rect_.x + points[i].x * displayed_frame_rect_.width /
                    displayed_frame_size_.width,
                displayed_frame_rect_.y + points[i].y * displayed_frame_rect_.height /
                    displayed_frame_size_.height);
            cv::Point2f b(
                displayed_frame_rect_.x + points[next].x * displayed_frame_rect_.width /
                    displayed_frame_size_.width,
                displayed_frame_rect_.y + points[next].y * displayed_frame_rect_.height /
                    displayed_frame_size_.height);
            cv::Point2f segment = b - a;
            double len2 = segment.dot(segment);
            double t = len2 > 0.0
                ? std::clamp((cv::Point2f(x, y) - a).dot(segment) / len2, 0.0, 1.0)
                : 0.0;
            cv::Point2f projection = a + segment * static_cast<float>(t);
            double distance = std::hypot(x - projection.x, y - projection.y);
        ...
        svc_->state.roi_params.selected_edge = nearest;
        roi_dragging_ = true;
        roi_drag_edge_ = nearest;
        roi_last_drag_frame_ = canvas_to_frame(x, y);
        return true;
    }

    if (event == cv::EVENT_MOUSEMOVE && roi_dragging_) {
        cv::Point2f current = canvas_to_frame(x, y);
        cv::Point2f delta = current - roi_last_drag_frame_;
        if (svc_->move_roi_edge_from_frame(roi_drag_edge_, delta, displayed_frame_size_))
            roi_last_drag_frame_ = current;
        return true;
    }
