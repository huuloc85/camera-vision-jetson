...
ImageProcessor::ImageProcessor(const DetectionParams &params, const RoiParams& roi)
  ...
  if (cv::cuda::getCudaEnabledDeviceCount() > 0) {
    try {
      ...
      cv::cuda::warpPerspective(gpu_frame_, gpu_warp_, identity, {1080, 804});
      cv::cuda::warpPerspective(gpu_frame_, gpu_roi_, identity, {540, 402});
      cv::cuda::cvtColor(gpu_roi_, gpu_gray_, cv::COLOR_BGR2GRAY);
      gpu_gaussian_->apply(gpu_gray_, gpu_blurred_);
      cv::cuda::threshold(gpu_blurred_, gpu_thresh_, params_.threshold, 255,
                          cv::THRESH_BINARY);
      gpu_morph_close_large_->apply(gpu_thresh_, gpu_morph_);
      gpu_morph_open_xl_->apply(gpu_morph_, gpu_thresh_);
      cv::Mat warmup_result;
      gpu_thresh_.download(warmup_result);
      log_msg(LOG_WARNING, "CUDA warm-up complete: warp + preprocess");
...
std::vector<cv::Point> ImageProcessor::map_roi_contour_to_frame(
    const std::vector<cv::Point>& contour, const cv::Size& frame_size,
    const cv::Size& roi_size) {
  if (contour.empty() || frame_size.width <= 0 || frame_size.height <= 0)
    return {};
  ensure_roi_transforms(frame_size);
  const bool fast_roi = roi_size == cv::Size(540, 402);
  const cv::Mat& transform = fast_roi ? M_half_ : M_;
  if (transform.empty())
    return {};

  std::vector<cv::Point2f> roi_points;
  roi_points.reserve(contour.size());
  for (const auto& point : contour) {
    cv::Point2f p(static_cast<float>(point.x),
                  static_cast<float>(point.y));
    if (DetectionConfig::ROTATE_ROI_180 &&
        !DetectionConfig::ROTATE_ROI_USING_SRC_REMAP) {
      p.x = static_cast<float>(roi_size.width - 1) - p.x;
      p.y = static_cast<float>(roi_size.height - 1) - p.y;
    }
    roi_points.push_back(p);
  }

  std::vector<cv::Point2f> frame_points;
  cv::perspectiveTransform(roi_points, frame_points, transform.inv());
  std::vector<cv::Point> mapped;
  mapped.reserve(frame_points.size());
  for (const auto& point : frame_points)
    mapped.emplace_back(cvRound(point.x), cvRound(point.y));
  return mapped;
}
