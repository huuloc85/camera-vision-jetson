...
#include "core/jetson_platform.h"
...
bool VisionService::set_roi_rectangle_from_frame(const cv::Point2f& first,
                                                 const cv::Point2f& second,
                                                 const cv::Size& frame_size) {
    if (frame_size.width <= 0 || frame_size.height <= 0)
        return false;

    const cv::Point2f ref_first =
        processor_.reference_point_from_frame(first, frame_size);
    const cv::Point2f ref_second =
        processor_.reference_point_from_frame(second, frame_size);
    const float left = std::min(ref_first.x, ref_second.x);
    const float right = std::max(ref_first.x, ref_second.x);
    const float top = std::min(ref_first.y, ref_second.y);
    const float bottom = std::max(ref_first.y, ref_second.y);
    if (right - left < 20.0f || bottom - top < 20.0f)
        return false;

    RoiParams candidate = state.roi_params;
    candidate.points = {{{left, top}, {right, top},
...
// ══════════════════════════════════════════════════════
cv::Mat VisionService::request_realtime_frame(double not_before) {
    if (!realtime_capture_enabled_)
        return cv::Mat();

    {
        std::lock_guard<std::mutex> lock(realtime_frame_mutex_);
        // The camera worker may already have preserved the first frame after
